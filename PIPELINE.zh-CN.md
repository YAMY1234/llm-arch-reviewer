...
- **失败时保守处理：** 无法确定归属的 event 必须明确保留为 `unmapped`，不能为了填补空白而分配给附近节点。
...
- tensor/state data flow、symbolic shape 和 dtype；
...
7. 无法匹配或存在歧义的 event 明确保留为 `unmapped`，并记录原因。
...
- 每个 measured event 都属于以下状态之一：已映射、显式 fused/shared、framework overhead，或者带原因的 explicit unmapped。
...
- 每个 IR node 必须显示以下一种状态：measured、fused/shared、structural、state-only、not selected for this phase，或 unmapped-with-reason。无法解释的空时间属于失败。
