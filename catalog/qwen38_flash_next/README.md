# Qwen3.8-Flash-Next IR-first catalog

## Canonical identity

The published model name is `Qwen3.8-Flash-Next`, following the Qwen team's
[official repository](https://github.com/QwenLM/Qwen3.8-Flash-Next) and
[Hugging Face checkpoint](https://huggingface.co/Qwen/Qwen3.8-Flash-Next).
The canonical catalog ID is `qwen38_flash_next` and the public Viewer ID is
`qwen38_flash_next_v2`.
`qwen40_v2` is retained only as a Viewer URL alias for old bookmarks. Pinned
source symbols such as `Qwen4ExpForConditionalGeneration`, source paths such as
`qwen4_exp.py`, and raw trace filenames containing `qwen40` remain unchanged:
they identify the exact historical implementation or immutable evidence and
are not product labels.
...
Each current Qwen3.8-Flash-Next profile also owns a content-addressed `timeline.v1`
...
`models/qwen38_flash_next/build/build_qwen38_flash_next_topology_cudagraph_profile.py` is the
...
`models/qwen38_flash_next/profile_patches/prefill_first_admission.patch`: it clears only a
