...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default="dsv4",
        help="built docs model id opened by the printed viewer URL",
    )
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "allowlisted raw-trace root; repeatable; no raw traces are served "
            "when omitted"
        ),
    )
...
def main() -> int:
    args = parse_args()
    trace_roots = args.trace_root or []
    ...
    print(
        f"viewer: http://{args.host}:{args.port}/viewer.html?model={args.model}",
        flush=True,
    )
    root_summary = ", ".join(str(root.resolve()) for root in trace_roots) or "none"
    print(
